...
def sorted_matrix_search(matrix, target):
    row = 0
    col = 0
    while matrix[row][col] < target: #find the desired row
        row+=1
    
    while matrix[row][col]< target: #find the desired column
        col+=1
    
    if matrix[row][col]==target: #make sure element is in array
        return row,col
    else:
        return -1,-1
...
def pascals_triangle(i):
    ...
    for index in xrange(1, i + 1):
        pascalRow.append( int(pascalRow[index-1] *((i+1 - index)/float(index)) ) ) #algorithm for calculating elements given a row
...
def alive_people(people):
    ...
    for person in people:
        startYear = int(person[0:person.index(':')]) #fill array with birth, death, and number of living at birth
        duration = int(person[person.index(':')+2:len(person)])

        years.append( [startYear, startYear+duration, 1])
    
    years = sorted(years, key=lambda year: year[0] ) #sort by earliest to latest birth
    
    for index in xrange(0, len(years)): #if death > later births, increment number living at those births
        for item in xrange (index+1, len(years)):
            if (years[item][0] < years[index][1]):
                years[item][2] += 1
        
    years = sorted(years, key=lambda year: year[2] ) #sort by birth

    return years[len(years)-1] #get last element from sort
...
def number_to_text(number):
    ...
    if number == 0:
        return "zero" #deal with individual cases
    if number == 100:
        return "one hundred"
    if number == 10:
        return "ten"
    ...
    #rest are formulaic
    onesPlace = {0: "", 1: "one", 2: "two", 3: "three", 4: "four", 5: "five", 6: "six", 7: "seven", 8: "eight", 9: "nine"}
    tensPlace = {0: "", 2: "twenty", 3: "thirty", 4: "forty", 5: "fifty", 6: "sixty", 7: "seventy", 8: "eighty", 9: "ninety"}
    
    return (tensPlace [number/10] + " " + onesPlace[number%10]).strip()    
...
def rotate_matrix(matrix):
        ...
        for cols in xrange (len(matrix)-1 , -1 , -1): #access columns from right to left
            for rows in xrange (len(matrix)): #access rows from top to down
                row.append (matrix[rows][cols]) #up -> down and left -> right added
...
def largest_sum_subarray(arr):
    highest = arr[0:1] #go through all subarrays and see which yields largest
    for start in xrange (len(arr)):
        for end in xrange (start + 1, len(arr) + 1):
            piece = arr[start:end]
            if (sum(piece) > sum(highest)):
                highest = piece
...
def palindrome(str_in):
    ...
    i = 0
    j = -1
    while ( i in xrange(0, len(plain_string)/2 ) and match): #match the ends, if they do it's palindromic
...
def longest_palindrome_substring(word):
    ...
    for start in xrange (len(word)):
        for end in xrange (start + 1, len(word) + 1):
            ...
            if palindrome(piece) and (end-start) > len(longest_word): #go through all substrings 
                longest_word = piece #if we get a palindrome longer than what's stored, store it
...
def is_unique(word):
    ...
    aValue = 97
    zValue = 122
    #if the dictionary finds a letter it fails
...
def longest_unique_substring(word):
    ...
    for start in xrange (len(word)):
        for end in xrange (start + 1, len(word) + 1):
            piece = word[start:end] #test all substrings
            if is_unique(piece) and (end-start) > len(longest_word):
                longest_word = piece
            elif not is_unique(piece): #no need in proceeding the test if it's already not unique
                break
...
def three_sum(arr, target):
    import itertools
    result = []
    triples = itertools.combinations(arr, 3)
    #calc all sums and see which is largest
    for item in triples:
        if sum(item) == target:
            result.append(item)
